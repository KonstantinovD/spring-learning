...
Так в чем же причина назначения нескольких типов грантов одному и тому же клиенту на стороне сервера авторизации? То, что я видел в паре систем, которое я считаю плохой практикой и которой лучше избегать, — это совместное использование учетных данных клиента. Совместное использование учетных данных клиента означает, что разные клиентские приложения используют одни и те же учетные данные клиента.

В потоке OAuth 2 клиент, даже если это приложение, действует как независимый компонент, имеющий собственные учетные данные, которые он использует для идентификации себя. Поскольку вы не делитесь учетными данными пользователя, вы также не должны делиться учетными данными клиента. Даже если все приложения, определяющие клиентов, являются частью одной системы, ничто не мешает вам зарегистрировать их как отдельные клиенты на уровне сервера авторизации. Индивидуальная регистрация клиентов на сервере авторизации дает следующие преимущества:

- Предоставляет возможность аудита событий индивидуально из каждого приложения. Когда вы регистрируете события, вы знаете, какой клиент их сгенерировал.
- Это обеспечивает более сильную изоляцию. Потеря одной пары учетных данных затрагивает только одного клиента.
- Это позволяет разделить объем. Вы можете назначать разные области действия (предоставленные полномочия) клиенту, который получает токен определенным образом.

Разделение областей является фундаментальным, и неправильное управление им может привести к странным сценариям. Предположим, вы определили клиента, как показано в следующем фрагменте кода:
...
Этот клиент настроен для authorization_code и client_credentials grant types. Используя любой из них, клиент получает токен доступа, который предоставляет ему права на чтение. Что здесь странно, так это то, что клиент может получить один и тот же токен, либо аутентифицируя пользователя, либо используя только свои собственные учетные данные. Это не имеет смысла и это нарушение безопасности.

Бывает и так, что все клиенты в системе настроены на один и тот же список, содержащий все возможные типы грантов (и некоторые из них представляют собой строки, которые даже не существуют как типы грантов!). Это может привести к ошибкам. И да, вы можете написать конфигурацию, подобную той, что представлена в этом фрагменте кода:
...
Пока вы не попытаетесь использовать тип гранта "abracadabra", приложение действительно будет работать.
