package ch6_hibernate.p334_one_to_one_lazy_and_eager_fetching;

import ch6_hibernate.common.CommonUtils;
import org.springframework.boot.SpringApplication;
import org.springframework.boot.autoconfigure.SpringBootApplication;
import org.springframework.scheduling.annotation.EnableScheduling;


@EnableScheduling
@SpringBootApplication(scanBasePackages = { "ch6_hibernate.p334_one_to_one_lazy_and_eager_fetching", "ch6_hibernate.common" })
public class OneToOneFetchingMain {

    public static void main(String[] args) {
        CommonUtils.setProperties("p334_one_to_one_lazy_and_eager_fetching");
        SpringApplication.run(OneToOneFetchingMain.class, args);
    }

}
